...
head(locations_2002)
```
In 2002, state in the list `r locations_2002$locationabbr` were observed at 7 or more locations. 
...
locations_2010
```
In 2010, state in the list `r locations_2002$locationabbr` were observed at 7 or more locations. 
...
```{r}
ggplot(data, aes(x = year, y = avg_data_value, color = state, group = state)) +
  geom_line() +
